...
use flowcloudai_client::tts::{AudioFormat, TTSRequest};
...
async fn main() -> anyhow::Result<()> {
    ...
    // ── 方式 1：最简调用 + 播放 ──
    let result = tts
        .speak("qwen3-tts-flash", "你好，世界。", "Ethan")
        .await?;
    ...
    if let Some(ref url) = result.url {
        ...
        AudioDecoder::play_source(
            &AudioSource::Raw(result.audio.clone()),
            Some(&result.format),
        )
        .await?;
    }
