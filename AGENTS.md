...
本仓库文档索引：`core_ai_client/docs/README.md`。索引带状态标记（`现行` / `待复核` / `归档`），先看状态再决定要不要读。注意：本目录 3 篇文档均停在 2026-04-25，索引里标为 `待复核`，引用前对照源码确认。

**问题排查记录（问题 → 根因 → 方案）不放本仓**，统一在工作区根 `docs/devlog/`，索引 `docs/devlog/README.md`。本仓当前唯一相关条目是移动端 wasmtime 页对齐 `SIGABRT`（`2026-06-04-移动端插件-页对齐-sigabrt.md`），其结论是硬约束。
